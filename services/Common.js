...
const Common = {
  ...
  async compareTime(tokenData, email) {
    ...
    if (currentTime > expireIn) {
      await Common.getAccessToken('refresh_token', tokenData.refresh_token).then((tokendata) => {
        Common.saveUserInfo(tokendata, email)
      })
      let refreshAccessToken = fs.readFileSync(`./storage/hubspot/${email}.json`)
  ...
  async saveSFUserInfo(accessToken, userInfo, email, instanceUrl, refreshToken) {
    let currentDateTime = new Date()
    let tokenCreatedAt = currentDateTime.getTime() / 1000
    userInfo.created_at = tokenCreatedAt
    userInfo.access_token = accessToken
    userInfo.email = email
    userInfo.instanceUrl = instanceUrl
